## R Function to run Ensemble Kalman Filter and do forecast 
## Based on Evensen 2008 and Shaman J, Karspeck A (2012) Proc Natl Acad Sci USA 109: 20425-20430.
...
EnKF_rFC<-function(num_ens, tmstep, param.bound, obs_i=obs_i, ntrn, 
                    obs_vars=vars,tm.ini=273, tm.range=273:500){
  ## EnKF to run retrospecitve forecast of influenza
  ## Inputs: 
  ...
  for (tt in 1:ntrn){
    ...
    ens_obs=obs_i[tt]+rnorm(num_ens,0,sd=sqrt(obs_var)); # ensemble of observations, add noise to the observation
    xpost[H,,tt]=post_var*(xprior[H,,tt]/prior_var + ens_obs/obs_var)
    dy=xpost[H,,tt]-xprior[H,,tt]

    ###  Getting the covariance of the prior state space and
    ###  observations  (which could be part of state space, e.g. newI)
    rr=NULL;
    for (j in 1:dim(xprior[,,tt])[1]){
      C=cov(xprior[j,,tt],xprior[H,,tt])/prior_var;  # covariance/varance of x.obs
      rr=append(rr,C);
    }
    dx=rr%*%t(dy);
